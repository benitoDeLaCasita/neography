  ...
  class Rest
    module NodeIndexes
      ...
      def get_node_index(index, key, value)
        index = @connection.get("/index/node/%1$s/%2$s/%3$s" % [index, key, encode(value)]) || []
      ...
      def find_node_index_by_key_value(index, key, value)
        @connection.get("/index/node/%1$s/%2$s/%3$s" % [index, key, encode(value)]) || []
      end

      def find_node_index_by_query(index, query)
        @connection.get("/index/node/%1$s?query=%2$s" % [index, encode(query)]) || []
      end
      ...
      def remove_node_index_by_id(index, id)
        @connection.delete("/index/node/%1$s/%2$s" % [index, get_id(id)])
      end

      def remove_node_index_by_key(index, id, key)
        @connection.delete("/index/node/%1$s/%2$s/%3$s" % [index, get_id(id), key])
      end

      def remove_node_index_by_value(index, id, key, value)
        @connection.delete("/index/node/%1$s/%2$s/%3$s/%4$s" % [index, get_id(id), key, encode(value)])
      end

      def drop_node_index(index)
        @connection.delete("/index/node/%1$s" % [index])
      end
      ...
      def create_unique_node(index, key, value, properties = {})
        ...
        @connection.post("/index/node/%1$s?unique" % [index], options)
      end
      ...
      def get_or_create_unique_node(index, key, value, properties = {})
        ...
        @connection.post("/index/node/%1$s?uniqueness=%2$s" %  [index, 'get_or_create'], options)
      ...
      def create_or_fail_unique_node(index, key, value, properties = {})
        ...
        @connection.post("/index/node/%1$s?uniqueness=%2$s" %  [index, 'create_or_fail'], options)
