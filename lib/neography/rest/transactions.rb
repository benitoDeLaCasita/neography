  ...
  class Rest
    module Transactions
      ...
      def in_transaction(tx, statements = [])
        ...
        @connection.post("/transaction/%1$s" % [get_tx_id(tx)], options)
      end
      ...
      def commit_transaction(tx, statements = [])
        if (tx.is_a?(Hash) || tx.is_a?(Integer))
          ...
          @connection.post("/transaction/%1$s/commit" %  [get_tx_id(tx)], options)
        else
      ...
      def rollback_transaction(tx)
        @connection.delete("/transaction/%1$s" % [get_tx_id(tx)])
      end
